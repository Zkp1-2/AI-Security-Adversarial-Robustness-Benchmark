...
def main(cfg_path: str):
    ...
    for epoch in range(1, cfg["train"]["epochs"] + 1):
        model.train()
        pbar = tqdm(train_loader, desc=f"Train Clean [Epoch {epoch}/{cfg['train']['epochs']}]")
        ...
        print(f"Epoch {epoch}: clean test acc = {test_acc:.4f}")

    torch.save(
        {"model_state": model.state_dict(), "cfg": cfg},
        cfg["train"]["ckpt_clean"]
    )
    print("Saved:", cfg["train"]["ckpt_clean"])
